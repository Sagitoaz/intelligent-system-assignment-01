...
def test_models_metadata(client):
    ...
    assert [model["id"] for model in models] == ["diabetes", "house_price"]
...
def test_feature_schema_matches_saved_model(client):
    ...
    for key in ("diabetes", "house_price"):
        assert service.metadata[key]["expected_raw_features"] == list(
            service.models[key].feature_names_in_
        )


def test_model_is_not_refit_during_prediction(client, diabetes_demo_case, house_demo_case, monkeypatch):
    service = client.app.state.model_service
...
def test_knowledge_graph_unavailable_does_not_break_predictions(client, diabetes_demo_case):
